...
@implementation AMEngineLoadedController
...
+ (void) sendRequest:(NSDictionary*)postcriteria withURL:(NSString*) urlstring withTarget: (id) target andSelector: (SEL) afterloadselector
{
    ...
    NSString * post = [AMEngineLoadedController concatDictionary:postcriteria withPairDelimiter:@"&" andValueDelimiter:@"="];
    ...
    AMEngineNSURLDelegate* urldelegate = [[AMEngineNSURLDelegate alloc] initWithTarget:target andSelector:afterloadselector];    
...
+ (NSString*) concatDictionary:(NSDictionary*) dict withPairDelimiter:(NSString*) pairdelim andValueDelimiter:(NSString*) valuedelim
{
    ...
    for(NSString* key in dict)
    {
        ...
        count++;
        NSString* encodedkey = [AMEngineLoadedController encodeString:key];
        NSString* encodedvalue = [AMEngineLoadedController encodeString:[dict objectForKey:key]];
...
{  
    ...
    NSString *localizedPath = [[NSBundle mainBundle] pathForResource:@"AMEngineSettings" ofType:@"plist"];  
...
@implementation AMEngineNSURLDelegate
