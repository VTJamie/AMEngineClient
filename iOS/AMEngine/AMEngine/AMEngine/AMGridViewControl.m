...
- (id) initWithFrame:(CGRect)frame andJsonDefinition:(NSDictionary *)jsondefinition andRootDefinition:(NSDictionary *)root
{
    ...
    if (self) 
    {
        ...
        //   [AMEngineDataManager testAlert:[NSString stringWithFormat:@"%@", currentdata]];
...
- (void) forTableViewController: (AssistedTableView*) tableController didSelectRowAtIndexPath: (NSIndexPath*) indexPath
{
    ...
    if(transferinfo != nil)
    {
        UINavigationController* navcontroller = [[[(AMEngineLayoutView*)self.superview getTopParent] parentcontroller]  navigationController];
        AMEngineMain* newmain = [[AMEngineMain alloc] initWithMenu:[[(AMEngineLayoutView*)self.superview getTopParent] parentcontroller].menu showLogoutButton:NO];
        AMEngineLayoutView* layoutview = (AMEngineLayoutView*)self.superview;
...
- (void) commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(editingStyle == UITableViewCellEditingStyleDelete)
    {
     ...
     //   [postdata addEntriesFromDictionary:[[self getTopParent] getFieldValues]];        
        ...
        [AMEngineDataManager sendRequest:postdata withTarget:self andSelector:@selector(dataManager:deleteResponse:)];
    }
}

- (void) dataManager:(AMEngineDataManager*) datamanager deleteResponse:(NSDictionary*) response
{
    //NSLog(@"%@", response);
    AMEngineLayoutView* layoutview = (AMEngineLayoutView*)self.superview;
    [[layoutview getTopParent] refreshControls];
