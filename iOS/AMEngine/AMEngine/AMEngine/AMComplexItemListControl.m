...
- (void) addButtonClick
{
    ...
    [AMEngineDataManager sendRequest:postdata withTarget:self andSelector:@selector(dataManager:addButtonClickResponse:)];
...
{    
    AMEngineMain* newmain = [[AMEngineMain alloc] initWithMenu:nil showLogoutButton:NO];
    newmain.dialogparent = [(AMEngineLayoutView*)self.superview getTopParent].parentcontroller;
    ...
    [[(AMEngineLayoutView*)self.superview getTopParent].parentcontroller presentModalViewController:navcontrol animated:YES];
   // NSLog(@"%@", response);
}
