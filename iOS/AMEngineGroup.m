...
@implementation AMEngineConfirmTabView
...
@implementation AMEngineAccordionHeader
...
@implementation AMEngineTabButton
...
{      
    if( self.associatedgroup.hidden)
    {
        ...
        {
            if([[curview class] isSubclassOfClass:[AMEngineTabButton class]])
            {
                AMEngineTabButton* curtabbutton = (AMEngineTabButton*)curview;
...
- (id)initWithDefinition:(NSDictionary *)indefinition andParent:(AMEngineBaseUIView *)inparent
{
    ...
    if(self)
    {
        ...
        for(NSDictionary* jsonobj in childobjects)
        {
            ...
            {
                UIView* processedview = [[AMEngineGroup alloc] initWithDefinition:jsonobj andParent:self];
...
-(UIView*)processControl:(NSDictionary *)control
{
    ...
    {       
        return [[AMEngineTextControl alloc] initWithDefinition:control andParent:self];
    }   
    else  if([typestring compare:@"date"] == NSOrderedSame)
    {        
        return [[AMEngineDateControl alloc] initWithDefinition:control andParent:self];
    }
    else if([typestring compare:@"dropdownlist"] == NSOrderedSame || [typestring compare:@"multiselect"] == NSOrderedSame)
    {
        return [[AMEngineSelectControl alloc] initWithDefinition:control andParent:self];
    }
    
    else if([typestring compare:@"grid"] == NSOrderedSame)
    {
        return [[AMEngineGridControl alloc] initWithDefinition:control andParent:self];
    }       
    else if([typestring compare:@"button"] == NSOrderedSame)
    {
        return [[AMEngineButtonControl alloc] initWithDefinition:control andParent:self];
    }
    else if([typestring compare:@"lookup"] == NSOrderedSame)
    {
        return [[AMEngineLookupControl alloc] initWithDefinition:control andParent:self];
    }
    else if([typestring compare:@"autosuggest"] == NSOrderedSame)
    {
        return [[AMEngineAutoSuggestControl alloc] initWithDefinition:control andParent:self];
    } 
    return nil;
}
...
- (void) DoTableLayout
{
    ...
    {           
        ...
        {
            AMEngineBaseUIView* curview = [self.subgroups objectAtIndex:(row * numcolumns) + column];
...
- (void) DoAccordionLayout
{
    ...
    for(AMEngineBaseUIView* curview in self.subgroups)
    {        
        AMEngineAccordionHeader* headerbutton = [[AMEngineAccordionHeader alloc] initWithFrame:CGRectMake(0, height, curview.frame.size.width, 25)];       
...
- (void) DoWizardOrTabLayout
{
    ...
    for(AMEngineBaseUIView* curview in self.subgroups)
    {        
        AMEngineTabButton* headerbutton = [[AMEngineTabButton alloc] initWithFrame:CGRectMake(tabcurposition+2, 0, tabwidth, 50)];               
