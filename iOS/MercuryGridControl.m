...
@implementation MercuryGridViewCell
...
@implementation MercuryGridControl
...
{
    MercuryGridViewCell* newcell = (MercuryGridViewCell*)[tableView dequeueReusableCellWithIdentifier:@"Cell"];
    if(newcell == nil)
    {
        newcell = [[MercuryGridViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"Cell"];
    }
...
{
    MercuryGridControl* gridcontrol = [MercuryGridControl getGridControlWithSearchGroup:searchgroup underGroup:group];
    if(gridcontrol != nil)
    {
...
- (void) executeSearch
{
    ...
    [MercuryLoadedController sendRequest:postdata withURL:@"/SpotESPageT1.aspx" withTarget:self andSelector:@selector(getNewGridData:)];
}
...
{
    for (MercuryBaseUIView* curview in group.subgroups)
    {
        NSString* dictsearchgroup = [curview.definition objectForKey:@"searchgroup"];     
        if([[curview class] isSubclassOfClass:[MercuryGridControl class]]
           && 
           [dictsearchgroup compare:searchgroup] == NSOrderedSame)
        {
            return (MercuryGridControl*)curview;
        }
        else if([[curview class] isSubclassOfClass:[MercuryGroup class]])
        {
            MercuryGridControl* gridcontrolfound = [MercuryGridControl getGridControlWithSearchGroup:searchgroup underGroup:curview];
...
{
    id<MercuryLookupDelegate> lookupdelegate = [self.definition objectForKey:@"lookupdelegate"];
