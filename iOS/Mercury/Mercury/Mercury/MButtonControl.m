...
- (void) buttonClick
{
    ...
    if([clicktype compare:[AMConstants C:BUTTON_TYPE_SUBMIT]] == NSOrderedSame)
    {
        if([[[self.rootdefinition objectForKey:[AMConstants C:RESPONSE_BODY]] objectForKey:[AMConstants C:IS_DIALOG]] boolValue])
        {
            ...
            [postdata addEntriesFromDictionary:[[(MercuryLayoutView*)self.superview getTopParent] getFieldValues]];
            [MercuryStaticVariables fillDictionary:postdata withPersistDictionary:[MercuryStaticVariables sharedInstance].persist];
            [MercuryDataManager sendRequest:postdata withTarget:self andSelector:@selector(dataManager:dialogSaveResponse:)];
        }
        else 
        {
            NSMutableDictionary* postdata = [[NSMutableDictionary alloc] init];
            ...
            [postdata addEntriesFromDictionary:[[(MercuryLayoutView*)self.superview getTopParent] getFieldValues]];
            [MercuryStaticVariables fillDictionary:postdata withPersistDictionary:[MercuryStaticVariables sharedInstance].persist];
            [MercuryDataManager sendRequest:postdata withTarget:self andSelector:@selector(dataManager:responseSaveString:)];
        }
...
- (void) dataManager:(MercuryDataManager*) datamanager responseSaveString:(NSDictionary *) response
{
    ...
    if([[[response objectForKey:[AMConstants C:RESPONSE_BODY]] objectForKey:[AMConstants C:ERRORS]] count] > 0)
    {
        ...
        { 
            [MercuryDataManager testAlert:[curerror objectForKey:[AMConstants C:ERROR_MESSAGE]]];
        }
    }
    else
    {                                                          
        MercuryLayoutView* topparent = (MercuryLayoutView*)self.superview;
...
- (void) dataManager:(MercuryDataManager*) datamanager dialogSaveResponse:(NSDictionary *) response
{
    ...
    {                                                   
        [[(MercuryLayoutView*)self.superview getTopParent].parentcontroller.dialogparent.rootview refreshControls];
        [[(MercuryLayoutView*)self.superview getTopParent].parentcontroller dismissModalViewControllerAnimated:YES];
    }
}
