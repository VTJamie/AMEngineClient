...
@implementation MercuryDataManager
...
+ (MercuryDataManager*) sendRequest:(NSDictionary*)postcriteria withTarget: (id) target andSelector: (SEL) afterloadselector
{
    return [MercuryDataManager sendRequest:postcriteria withTarget:target andSelector:afterloadselector getString:YES];
}

+ (MercuryDataManager*) sendRequest:(NSDictionary*)postcriteria withTarget: (id) target andSelector: (SEL) afterloadselector getString:(BOOL) transformtostring
{
    NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
    NSString* urlstring = [userDefaults stringForKey:@"base_url"];
    
    NSString * post = [MercuryDataManager concatDictionary:postcriteria withPairDelimiter:@"&" andValueDelimiter:@"="];
    NSLog(@"URL: %@", urlstring);
    NSLog(@"%@", postcriteria);
    ...
    MercuryDataManager* urldelegate = [[MercuryDataManager alloc] initWithTarget:target andSelector:afterloadselector andTransformToString:transformtostring];
...
+ (NSString*) concatDictionary:(NSDictionary*) dict withPairDelimiter:(NSString*) pairdelim andValueDelimiter:(NSString*) valuedelim
{
    ...
    for(NSString* key in dict)
    {
        ...
        count++;
        NSString* encodedkey = [MercuryDataManager encodeString:key];
        NSString* encodedvalue = [MercuryDataManager encodeString:[dict objectForKey:key]];
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection
{
    ...
    if(self.transformtostring)
    {
        ...
        if([self.target respondsToSelector:self.finishloading])
        {
            ...
            [MercuryDataManager handleResponseStatus: responsedict];
...
+ (void) handleResponseStatus:(NSDictionary*) response
{
    if([AMConstants isLoaded])
    {
        if(![[response objectForKey:[AMConstants C:SESSION_VALID]] boolValue])
        {
            ...
            {
                [MercuryDataManager testAlert:[NSString stringWithFormat:@"%@", [response objectForKey:[AMConstants C:ERROR]]]];
            }
        }
        NSLog(@"%@", [response objectForKey:[AMConstants C:PERSIST]]);
        [MercuryStaticVariables sharedInstance].persist = [response objectForKey:[AMConstants C:PERSIST]];
    }
}
