...
@implementation AMEngineLookupCustomView
-(void) layoutSubviews
{
    NSInteger curxpos = 0;
    NSInteger curypos = 0;
...
@implementation AMEngineLookupControl
@synthesize label;
...
- (id) initWithDefinition:(NSDictionary*) indefinition andParent:(AMEngineGroup *)inparent
{
    ...
    if (self)
    {
        ...
        else if ([self.definition objectForKey:@"ts"] != nil) 
        {
            ...
            self.speciallookupview = [[AMEngineLookupCustomView alloc] initWithDefinition:[self.definition objectForKey:@"ts"] andParent:self];
...
- (void) buttonClick
{
    AMEngineViewController* skilltable = [[AMEngineViewController alloc] initWithLookupDefinition:self.definition andLookupSelectDelegate:self];
